...
| Cross-origin request (CORS) settings | Any website is allowed to call the API using a logged-in user's credentials | SonarQube (S5122 `CORSCheck` has a named matcher for Spring's `CorsConfiguration.setAllowedOriginPatterns`), OWASP ZAP (passive scan on live response headers) — **not Semgrep**: confirmed by checking its actual rule source (`java.lang.security.audit.permissive-cors`), which only matches raw `HttpServletResponse.setHeader`/`addHeader` calls, not Spring's `CorsConfiguration` bean pattern; the Spring-specific ruleset (`java/spring/security/audit/`) has no CORS rule at all. A genuine Semgrep coverage gap for this vulnerability shape, not a pipeline misconfiguration. |
